...
```python
{
  ...
  "_meta": {
    ...
    "target_views": [...],
    "required_views": [...],          # policy 标记的 required，**只取 4 个目标 view 的交集**
    "empty_required_views": [...],    # required 中过滤后为空的 view 列表（业务事实暴露）
    "preflight_report_sha256": "<sha256 of governance report>",  # KS-COMPILER-013 报告版本指纹
  }
}
```

**关于 `empty_required_views`**：required view 命中 0 行是**真实业务事实**（如 W6 阶段 `content_type_view.coverage_status` 全为 `missing`，被 policy 的 `coverage_status ∈ [complete, partial]` 过滤后必然为空）。本卡是 thin retrieval，**不在此处 raise / 不做 fallback 决策**（决策属 KS-RETRIEVAL-007 `fallback_decider` 的范畴 → 走 `blocked_missing_required_brand_fields` / `domain_only` 等显式降级），但必须把空集事实显式暴露到 `_meta.empty_required_views`，让下游能追责、能 replay。**关键反模式**：静默把"required view 0 行"当成"没事"——会导致下游误以为知识齐全，绕过 fallback。
...
## 6. 对抗性 / 边缘性测试（≥ 15 case，强制覆盖；现状 21 case）
...
| T16 | `structured_filters` 实际应用（如 `coverage_status ∈ [complete,partial]`） | 按列过滤生效 |
| T17 | `allowed_layers` 非法（空 / 非 `domain_general\|brand_*`） | raise |
| T18 | `_meta.preflight_report_sha256` 存在 + 64 字符 hex + 同 report 调两次相同 | pass |
| T19 | 报告内容变化 → sha256 改变 | pass |
| T20 | required view（如 `content_type_view`）过滤后为空 | 不 raise；`_meta.empty_required_views` 列出该 view（W6 真实场景） |
| T21 | required view 全部命中 | `empty_required_views == []` |
...
E2 防假绿（参见 core error pattern）：`exit 0` 不等于通过——CI 必须断言 `passed >= 21 and skipped == 0`，否则视为 `fail + unverified`。

**CI 顺序硬约束**：新环境必须先跑 `python3 knowledge_serving/scripts/validate_serving_governance.py --all` 生成 `validate_serving_governance.report`，再跑本卡 preflight；否则 preflight fail-closed（这是预期行为，不是 bug）。
...
- [x] `structured_retrieval.py` 入 git（commit 5301242 + 本次收口 commit）
- [x] `test_struct_retrieval.py` 含 ≥ 15 case 全绿（实测 21 passed / 0 skipped / 0 failed）
- [x] `validate_serving_governance.report` 已存在且 S1-S7 全 PASS（preflight 通过 + sha256 写入 _meta）
- [x] dag.csv 中本卡 status 从 `not_started` → `done`
- [x] 独立审查员 pass（FAIL findings 全部修复：sha256 暴露 / empty_required_views 暴露 / 状态收口）
- [x] `git diff --stat clean_output/` 跑完后为空
- [x] frontmatter `writes_clean_output: false` 未被改动

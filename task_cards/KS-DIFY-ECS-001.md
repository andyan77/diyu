...
## 10. 独立审查员 Prompt
> 请：
> 1. 跑 `bash -c 'source scripts/load_env.sh && python3 scripts/verify_ecs_mirror.py --dry-run --env staging'`，必须 exit 0、drift_total=0、报告落 `_staging/ecs_mirror_check/<run_id>/mirror_drift_report.json`、未真改任何文件。**注意命令必须自带 `source scripts/load_env.sh`；空白 shell 直跑 python 会因缺 ECS env exit 2，那是审查环境问题不是脚本失败。**
...
> 5. `bash -c 'source scripts/load_env.sh && python3 scripts/verify_ecs_mirror.py --env prod --dry-run'` 必须 exit != 0（prod 拒绝先于 env 校验触发，无 env 也能拒绝）

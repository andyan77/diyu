...
- [x] API live vector path verified post-FIX-12 — runtime_verified（2026-05-14 KS-FIX-12 收口重 scope 后落地：删 `retrieve_context.py:213 vector_res = None` 硬编码 → 默认 `structured_only=False` 时真调 `vector_retrieve()`（真 dashscope text-embedding-v3 + 真 ECS Qdrant `ks_chunks_current` alias）；Qdrant 不可达 + mode=vector → 503 fail-closed `qdrant_unreachable`（不静默 None）；`structured_only=True` 显式开 → 兼容跳过 vector。三轨 e2e audit：`knowledge_serving/audit/retrieval_006_staging_KS-FIX-12.json`，verdict=pass / pass=3 fail=0 / C1 200 mode=vector candidate_count>0 / C2 200 mode=structured_only_opt_in / C3 503 qdrant_unreachable；pytest `knowledge_serving/tests/test_retrieval_006_staging.py` 4/4 pass；红线：S10 vector 召回在 API 主路径首次真实门控）

...
| 3 | W3 | KS-COMPILER-002 | done | ~~RISKY~~ → **RESOLVED 2026-05-14** | ~~Coverage is fully missing while command exits 0.~~ Closed by **KS-FIX-06** 路径 A：`test_compiler_coverage.py` pytest 真实断言（threshold=0.95），content_type_view ratio=1.0 covered=18/18，`compiler_coverage_KS-FIX-06.json` 含 e8_decision=`path_A_real_assertion` + F2 fail-closed. | ~~Decide whether `coverage=missing` is allowed.~~ Done. |
| 4 | W3 | KS-COMPILER-010 | done | ~~CONDITIONAL_PASS~~ → **RESOLVED 2026-05-14** | ~~Rerank is declared in policy but not actually called.~~ Closed by **KS-FIX-21** 路径 B（用户审查员裁决换企业叙事 10 query）：真调 DashScope gte-rerank，**rerank_invoked=10/10, topk_order_changed=10/10**，pass_condition_met=True，`rerank_runtime_KS-FIX-21.json` evidence_level=runtime_verified. | ~~Validate rerank in runtime acceptance.~~ Done. |
